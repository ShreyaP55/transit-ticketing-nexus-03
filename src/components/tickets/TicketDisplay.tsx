...
import { Clock, MapPin, TicketX, Ticket } from "lucide-react";
...
const TicketDisplay: React.FC = () => {
  ...
  // Filter for valid tickets only (purchased within 7 hours)
      ...
      <Card className="bg-card border-border shadow-md">
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-medium mb-1 text-card-foreground">No Valid Tickets</h3>
            <p className="text-muted-foreground">
              You don't have any valid tickets at the moment
            </p>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <Card className="bg-card border-border shadow-md">
      <CardHeader>
        <CardTitle className="flex items-center text-card-foreground">
          <Ticket className="mr-2 h-5 w-5 text-primary" />
          Your Tickets ({validTickets.length})
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4 max-h-96 overflow-y-auto">
          {validTickets.map((ticket: ITicket) => (
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-muted-foreground">
                      Purchased: {format(new Date(ticket.purchasedAt), "MMM d, yyyy h:mm a")}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-muted-foreground">
                      Expires: {format(new Date(ticket.expiresAt), "MMM d, yyyy h:mm a")}
                    </span>
                  </div>
                </div>
